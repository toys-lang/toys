...
public class InterpreterTest {
    ...
    public void testWhile() {
        ...
        Ast.Expression program = Block(
                Assignment("i", integerLiteral(0)),
                While(
                        lessThan(symbol("i"), integerLiteral(10)),
                        Block(
                                Println(symbol("i")),
                                Assignment("i", add(symbol("i"), integerLiteral(1)))
                        )
                )
        );
    ...
    public void testFactorial() {
        List<TopLevel> topLevels = List.of(
                ...
                DefineFunction("main", List.of(), Block(
                        Println(call("fact", integerLiteral(5)))
                )),
                // def factorial(n) {
                //   if(n < 2) {
                //     1
                ...
                DefineFunction("fact", List.of("n"), Block(
                        If(
                                lessThan(symbol("n"), integerLiteral(2)),
                                integerLiteral(1),
                                Optional.of(
                                        (Ast.Expression)multiply(symbol("n"), call("fact", subtract(symbol("n"), integerLiteral(1))))
                                )
                        )))
        );
